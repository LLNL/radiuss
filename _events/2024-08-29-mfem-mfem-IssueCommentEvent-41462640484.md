...
<small>> This is really strange, maybe check if you are still not getting to the parallel version of `GetEssentialVDofs` somehow? Could be a compiler error with the default argument maybe? It is a wild theory, but does `GetEssentialVDofs(bdr, all_bdr_ldofs_marked, -1)` make difference?...</small>
